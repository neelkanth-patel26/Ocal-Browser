...
(function() {
    ...
    const injectButton = () => {
        if (document.getElementById('ocal-sidebar-close-btn')) return;

        const closeBtn = document.createElement('button');
        closeBtn.id = 'ocal-sidebar-close-btn';
        ...
        closeBtn.style.cssText = `
            ...
            background: rgba(8, 8, 9, 0.75) !important;
        ...
        closeBtn.innerHTML = `
            <svg width="10" height="10" viewBox="0 0 12 12" style="display: block !important; pointer-events: none !important;">
                <path d="M1.5 1.5L10.5 10.5M10.5 1.5L1.5 10.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" fill="none"/>
            </svg>
        `;

        // Style overrides on interaction
        closeBtn.addEventListener('mouseenter', () => {
            closeBtn.style.setProperty('background', 'rgba(239, 68, 68, 0.95)', 'important');
            closeBtn.style.setProperty('border-color', 'rgba(239, 68, 68, 0.95)', 'important');
            closeBtn.style.setProperty('color', '#ffffff', 'important');
            closeBtn.style.setProperty('transform', 'scale(1.06)', 'important');
        });
        
        closeBtn.addEventListener('mouseleave', () => {
            closeBtn.style.setProperty('background', 'rgba(8, 8, 9, 0.75)', 'important');
            closeBtn.style.setProperty('border-color', 'rgba(255, 255, 255, 0.15)', 'important');
            closeBtn.style.setProperty('color', '#e8e8e8', 'important');
            closeBtn.style.setProperty('transform', 'scale(1)', 'important');
        });

        // Prevent click events from propagating to underlying page
        ...
        closeBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            ipcRenderer.send('close-all-sidebars');
        });

        // Insert into documentElement so it works before body is ready
